...
bool Tracker::initializeCapture() { return cap_.isOpened(); }
...
std::vector<std::vector<float>> Tracker::pixelToCameraFrame(
    const std::vector<cv::Point>& prediction_pixels) {
  std::vector<std::vector<float>> coordinates;
  for (const auto& pixel : prediction_pixels) {
    float offset_y =
        (pixel.y - (static_cast<float>(resolution_[1]) / 2)) * pixel_size_;
    float dip_angle =
        (vfov_ / 2) - radiansToDegrees(std::atan2(offset_y, focal_length_));
    float z = height_ / std::tan(degreesToRadians((vfov_ / 2) - dip_angle));
    float x_offset =
        (pixel.x - static_cast<float>(resolution_[0]) / 2) * pixel_size_;
...
cv::Mat Tracker::plotCoordinates(
    const std::vector<cv::Point>& prediction_pixels,
    const std::vector<std::vector<float>>& coordinates, cv::Mat& frame) {
  for (size_t i = 0; i < prediction_pixels.size(); ++i) {
    cv::Point pt = prediction_pixels[i];
    cv::circle(frame, pt, 3, cv::Scalar(0, 255, 0), 2);
    std::string text = "(" + std::to_string(coordinates[i][0]) + ", " +
                       std::to_string(coordinates[i][1]) + ", " +
                       std::to_string(coordinates[i][2]) + ")";
    cv::putText(frame, text, pt, cv::FONT_HERSHEY_SIMPLEX, 0.5,
                cv::Scalar(125, 246, 55), 1);
  }
  return frame;
}
