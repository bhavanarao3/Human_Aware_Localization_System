...
// Using specific declarations instead of the whole namespace.
...
const Scalar BLUE(255, 178, 50);
const Scalar YELLOW(0, 255, 255);
const Scalar RED(0, 0, 255);
const int NUM_ROWS = 25200;  // Define as a constant for better readability
const int DIMENSIONS = 85;   // Define as a constant for better readability

/**
 * @brief Constructor for the Detector class. Initializes the YOLO model and
 * loads class names.
...
/**
 * @brief Preprocesses the input image by creating a blob to feed into the
 * neural network.
...
Mat Detector::PreProcess(const Mat& input_image) {
  Mat blob;
  dnn::blobFromImage(input_image, blob, 1. / 255.,
                     Size(INPUT_WIDTH, INPUT_HEIGHT), Scalar(), true, false);
  net_.setInput(blob);
  return blob;
}

/**
 * @brief Postprocesses the neural network output and draws bounding boxes on
 * the input image.
...
Mat Detector::PostProcess(Mat& input_image, const vector<Mat>& outputs) {
  ...
  float* data =
      reinterpret_cast<float*>(outputs[0].data);  // Use reinterpret_cast
  ...
  for (int i = 0; i < NUM_ROWS; ++i) {
    ...
    if (confidence >= CONFIDENCE_THRESHOLD) {
      ...
      minMaxLoc(Mat(1, class_names_.size(), CV_32FC1, classes_scores), 0,
                &max_class_score, 0, &class_id);
      ...
      if (max_class_score > SCORE_THRESHOLD && class_id.x == 0) {
        ...
        int left =
            static_cast<int>((cx - 0.5f * w) * x_factor);  // Use static_cast
        int top =
            static_cast<int>((cy - 0.5f * h) * y_factor);  // Use static_cast
        int width = static_cast<int>(w * x_factor);        // Use static_cast
        int height = static_cast<int>(h * y_factor);       // Use static_cast
        boxes.push_back(Rect(left, top, width, height));
      }
    }
    data += DIMENSIONS;  // Use the defined constant
  }
 ...
 */
void Detector::DrawLabel(Mat& input_image, const string& label, int left,
                         int top) {
  int baseLine;
  ...
  putText(input_image, label, Point(left, top + label_size.height),
          FONT_HERSHEY_SIMPLEX, 0.7, YELLOW, 1);
}

/**
 * @brief Detects objects in the input image using YOLO and returns the image
 * with bounding boxes.
