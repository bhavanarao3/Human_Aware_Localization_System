...
int main(int argc, char** argv) {
  if (argc != 5) {
    std::cerr << "Usage: ./shell-app <model_path> <class_names_path> "
  ...
  std::string droidcam_url = "http://" + droidcam_ip + ":" + droidcam_port +
                             "/video";  // Adjust the path if necessary.
  ...
  Tracker tracker(height, focal_length, hfov, vfov, resolution, pixel_size,
                  droidcam_url);

  // Ensure capture is initialized.
  if (!tracker.initializeCapture()) {
    std::cerr << "Error: Could not initialize capture from DroidCam."
              << std::endl;
    return -1;
  }
  ...
  while (true) {
    ...
    // Assume predictions are returned as bounding boxes; extract their center
    // points.
    ...
    output_image =
        tracker.plotCoordinates(prediction_pixels, coordinates, output_image);
